...
#
$MATLAB/bin/matlab-glselector.sh "$nodisplay" "$noopengl" "$batchflag" "$usemesa" "$nousemesa" "$display" "$ARCH" "$MATLAB"
glselector=$?
...
#
    ...
    glnx*)
        ...
        # g1952175 - Load the correct glib libraries needed by CEF
        # Use our shipped glib binaries on distros that ship with older versions of glib.
        sys_glib=`"$MATLAB/bin/$ARCH/glib_version"`
        LD_LIBRARY_PATH_OLD=$LD_LIBRARY_PATH
        LD_LIBRARY_PATH=$MATLAB/cefclient/sys/os/glnxa64:$LD_LIBRARY_PATH
        export LD_LIBRARY_PATH # Just in case this var isn't already part of the user env.
        cef_glib=`"$MATLAB/bin/$ARCH/glib_version"`
        newer_glib="`printf '%s\\n' $sys_glib $cef_glib | sort -Vr | head -n 1`"
        if [ $newer_glib = $sys_glib ]; then
            # The system glib is newer, revert changes to LD_LIBRARY_PATH
